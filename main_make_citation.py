...
def run(excel_path: Path) -> int:
    ...
    sheet_initial_setup = xr.xread("cell", 10, 4, sheet = 1)
    sheet_url_list_path = xr.xread("cell", 11, 4, sheet = sheet_initial_setup)

    sheet_search_targets_3gpp = xr.xread("cell", 12, 4, sheet = sheet_initial_setup)
    sheet_search_targets_ieee = xr.xread("cell", 13, 4, sheet = sheet_initial_setup)
    sheet_search_rules_3gpp = xr.xread("cell", 14, 4, sheet = sheet_initial_setup)
    sheet_search_rules_ieee = xr.xread("cell", 15, 4, sheet = sheet_initial_setup)


    proxy_url = xr.xread("cell", 1, 3, sheet = sheet_initial_setup)
    download_dir = xr.xread("cell", 2, 3, sheet = sheet_initial_setup)
    database_url = xr.xread("cell", 3, 3, sheet = sheet_initial_setup)
    ...
    download_urls =  xr.xread("col", 1, header=True, sheet = sheet_url_list_path)
